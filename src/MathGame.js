...
const generateQuestion = () => {
  ...
  // Set the image name based on the result of the operation
  setCurrentImageName(`${tempNum1}`);

  setQuestion(questionText);
  setUserAnswer('');
  setIsCorrect(null); // Reset isCorrect to null when a new question is generated
};
...
const checkAnswer = () => {
  try {
    ...
    if (!isNaN(userNum)) {
      ...
      if (isCorrect) {
        // Set the image name for the correct answer
        setCurrentImageName(`${correctAnswer}`);
        // Generate a new question for correct answers
        generateQuestion();
...
return (
  <View style={styles.container}>
    {/* Display the image based on the currentImageName */}
    {isCorrect !== null && currentImageName && (
      <Image source={numberImages.find(img => img.name === currentImageName)?.image} style={styles.image} />
    )}
