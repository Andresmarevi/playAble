...
const MathGame = () => {
  ...
  const generateQuestion = () => {
    let tempNum1, questionImages, isInvalidResult;

    do {
      tempNum1 = Math.floor(Math.random() * 11);
      isInvalidResult = false;
      questionImages = [];
      let currentNumber = tempNum1;

      if (correctAnswersCount >= 10) {
        const operatorCount = correctAnswersCount >= 20 ? 3 : 2;

        for (let i = 0; i < operatorCount; i++) {
          const tempNum2 = Math.floor(Math.random() * 11);
          const operator = getOperator();
          questionImages.push({ image: numberImages[tempNum2 - 1].image, name: `${tempNum2}` });
          questionImages.push({ text: operator });
          currentNumber = applyOperator(currentNumber, tempNum2, operator);
        }
      }

      while (currentNumber > 0) {
        const digit = currentNumber % 10;
        questionImages.unshift({ image: numberImages[digit - 1].image, name: `${digit}` });
        currentNumber = Math.floor(currentNumber / 10);
      }
    } while (isInvalidResult);

    setQuestion(questionImages);
    setUserAnswer('');
  };
    ...
    try {
      ...
      if (!isNaN(userNum)) {
        const parts = question
          .map((item) => (item.name ? item.name : item.text))
          .join(' ');

        const correctAnswer = evaluate(parts);
        const isCorrect = Math.abs(userNum - correctAnswer) < 0.0001;
        ...
        if (isCorrect) {
          setCorrectAnswersCount((count) => (count < 30 ? count + 1 : 30));
        } else {
          setCorrectAnswersCount(0);
        }
      } else {
  ...
  useEffect(() => {
    if (correctAnswersCount === 30) {
      Alert.alert(
        'Congratulations!',
        'You completed 30 correct answers!',
        [
          {
            text: 'OK',
            onPress: () => {
      ...
      <View style={styles.question}>
        {question.map((item, index) => (
          <React.Fragment key={index}>
            {item.image && (
              <Image key={index} source={item.image} style={styles.numberImage} />
            )}
            {item.text && <Text key={index}>{item.text}</Text>}
          </React.Fragment>
        ))}
      </View>
      <TextInput
